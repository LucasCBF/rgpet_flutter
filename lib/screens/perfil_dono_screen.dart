...
class PerfilDonoScreen extends StatefulWidget {
  const PerfilDonoScreen({super.key});

  @override
  State<PerfilDonoScreen> createState() => _PerfilDonoScreenState();
}

class _PerfilDonoScreenState extends State<PerfilDonoScreen> {
  Map<String, dynamic>? _donoData;
  ...
  Future<void> _loadDonoProfile() async {
    ...
    try {
      DocumentSnapshot<Map<String, dynamic>> docSnapshot =
          await FirebaseFirestore.instance.collection('donos').doc(user.uid).get();

      if (docSnapshot.exists) {
        setState(() {
          _donoData = docSnapshot.data();
          _isLoading = false;
        });
      } else {
        setState(() {
          _errorMessage = 'Dados do perfil do dono não encontrados.';
          _isLoading = false;
        });
        print('Documento do dono não encontrado no Firestore para UID: ${user.uid}');
      }
    } catch (e) {
      setState(() {
      ...
      print('Erro ao carregar perfil do dono: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar( // Adicionando AppBar aqui, pois estava faltando na tela de perfil
        title: Text(
          'Meu Perfil',
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildProfileInfoTile(
                            ...
                            icon: Icons.person,
                            label: 'Nome Completo',
                            value: _donoData!['nomeCompleto'] ?? 'N/A',
                          ),
                          ...
                          _buildProfileInfoTile(
                            ...
                            label: 'Membro desde',
                            value: (_donoData!['dataCadastro'] as Timestamp?)?.toDate().toLocal().toString().split(' ')[0] ?? 'N/A',
                          ),
